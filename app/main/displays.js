...
const makeWindow = (display) => {
  ...
  win.loadURL(format({
    protocol: 'file',
    slashes: true,
    pathname: resolve(__dirname, '../background/index.html')
  }))
...
const onDisplayChange = (e, display) => {
  ...
  for (let i = keys.length - 1; i > -1; i -= 1) {
    if (windows[keys[i]].display.id === display.id) {
      isAllowedResize = false
      return windows[keys[i]].setBounds(display.bounds, false)
    }
  }
}
...
const initDisplays = () => {
  ...
  for (let i = displays.length - 1; i > -1; i -= 1) {
